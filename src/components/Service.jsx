...
const Service = ({ image, format, title, subContent, discribe }) => {
  ...
  const ServiceInnerContainer = styled.div`
    ...
    flex-direction: ${props =>
      props.format == "reverse" ? "row-reverse" : "row"};
  ...
  const ImageCon = styled.div`
    ...
    display: flex;
    // border: 2px solid red;
    margin-right: ${props => (props.format == "reverse" ? "0rem" : "3rem")};
    margin-left: ${props => (props.format == "reverse" ? "3rem" : "0rem")};
    @media (max-width: ${theme.tablet}) {
      width: 100%;
      margin: 0;
    }
...
`
  ...
  return (
    <ServiceContainer>
      <ServiceInnerContainer format={format}>
        <ImageCon format={format}>
          <CoverImage>
